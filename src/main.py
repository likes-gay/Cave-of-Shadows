...
GAME_WORLD = {
	...
	"Side Path":{
		...
		"options": [
			{"next": "Take the narrow path", "next": "Narrow Path - Side Entrance"},
			{"next": "Take the winding path", "next": "Winding Path"}
		],
	},
...
def print_typewriter_effect(text: str, effect: str):
	if effect == "Slow":
		for char in text:
...
class PlayerData():
	...
	def _save_game(self, override: list[PlayerDataType] | None = None):
		...
		if override is None:
			...
			game_index = next([i for i, player_data in enumerate(all_player_datas) if player_data["game_name"] == self.game_name])
			if game_index:
				all_player_datas[game_index] = self.__dict__
			else:
	...
	def _get_typewriter_option(self):
		...
		option = get_valid_arr_input(
			...
			[f"{Fore.GREEN}{plain_options_arr[0]}{Style.RESET_ALL}", f"{Fore.YELLOW}{plain_options_arr[1]}{Style.RESET_ALL}", f"{Fore.WHITE}{Style.RESET_ALL}"]
		)
